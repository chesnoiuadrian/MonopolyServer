...
import com.ea.ja.monopoly.message.*;
...
final class Server implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (isRunning) {
                ...
                if(currentConnectedClients < MAXIMUM_NUMBER_OF_CLIENTS && currentConnectedClients < requiredClients)
                    if (true) {
                        ...
                        clients.add(new Client(username,socket,objectInputStream,objectOutputStream));
            ...
            e.printStackTrace();
        } catch (InvalidRequstedCode invalidRequstedCode) {
